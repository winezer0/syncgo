...
- **`syncgo.exe`** — Windows 主程序
- **`syncgo_linux`** — Linux 远程 agent（通过 `deploy-agent` 或 TUI 部署）
...
```powershell
.\syncgo.exe                        # 双击进 TUI
.\syncgo.exe tui                    # 命令行启动 TUI
.\syncgo.exe init                   # 生成配置模板
.\syncgo.exe list                   # 列出任务/服务器
.\syncgo.exe test myserver          # 测试 SSH 连接
.\syncgo.exe push web               # 一键同步
.\syncgo.exe push --dry-run         # 模拟运行，预览变更
.\syncgo.exe deploy-agent myserver  # 部署远端 delta agent
.\syncgo.exe exec myserver "df -h"  # 远端执行命令
.\syncgo.exe exec --all "uptime"    # 所有服务器执行
.\syncgo.exe version                # 版本信息
```

> 双击 `syncgo.exe` 进入 TUI 即可创建配置，无需手写 YAML。
...
| `syncgo test <server>` | 测试 SSH 连接 |
| `syncgo init` | 生成配置模板 |
| `syncgo exec <server> "cmd"` | 远端执行命令 |
| `syncgo exec --all "cmd"` | 所有服务器执行 |
| `syncgo exec <server> --file script.sh` | 从文件读取命令 |
| `syncgo deploy-agent <server>` | 部署远端 delta agent |
| `syncgo version` | 版本信息 |
...
`syncgo deploy-agent` 自动完成远端 delta agent 的部署：
...
Agent 与主程序是**同一个二进制**（同一份源码 `cmd/syncgo`）。远端仅使用其中的 `receive` 子命令进行 delta 签名计算和文件重建，但部署的是包含所有功能的完整 syncgo 可执行文件。

### 二进制获取策略（三级回退）

| 优先级 | 来源 | 说明 |
|--------|------|------|
| 1 | 本地文件 | 程序目录下的 `syncgo_linux_<arch>`（如 `syncgo_linux_amd64`） |
| 2 | GitHub Releases | 自动下载 `v<version>/syncgo_linux_<arch>` |
| 3 | 交叉编译 | 本机 `go build`（需 Go 环境，`CGO_ENABLED=0` 静态编译） |

> 提示：将预构建的 `syncgo_linux_amd64` / `syncgo_linux_arm64` 放在 `syncgo.exe` 同级目录，即可离线部署，无需网络和 Go 环境。

### 执行步骤

1. 连接服务器，检测 CPU 架构（`uname -m`）
2. 按三级回退策略获取 agent 二进制
3. SFTP 上传至 `~/.local/bin/syncgo`
4. 设置可执行权限（`chmod 0755`）
5. 确保 `~/.local/bin` 在 PATH 中
6. **执行验证** — 远端运行 `syncgo version` 确认可执行
...
部署后，`syncgo push` 自动使用 delta 增量传输。

## 库化调用（Go SDK）

syncgo 可作为 Go 库嵌入第三方项目：
...
远端 agent 将块签名缓存至 `~/.syncgo_cache/`，文件未变化时跳过读盘直接返回缓存签名，大幅加速重复同步。
