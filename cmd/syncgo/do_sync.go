...
func doAdHocSync(source, target string, delete, flat, checksum bool, exclude []string, cfgPath string, dryRun, verbose bool, workers int, algoName string) {
	...
	stats, syncErr := engine.Sync(transport.SyncOptions{
		Source:   source,
		Target:   remotePath,
		Delete:   delete,
		Exclude:  exclude,
	...
	if dryRun {
		fmt.Println("Dry run complete — remove --dry-run to sync")
	}
}

// printVerboseStats prints detailed sync statistics.
func printVerboseStats(stats *transport.SyncStats, dryRun, verbose bool) {
	if verbose && stats.DeltaSaved > 0 {
		fmt.Printf("  delta-matched: %s\n", util.FormatBytes(stats.DeltaSaved))
	}
